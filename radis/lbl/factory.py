...
class SpectrumFactory(BandFactory):
    ...
    def eq_spectrum_with_uncertainty(
        self,
        Tgas,
        n_samples=200,
        uncertainty_source="hitran",
        confidence_level=0.95,
        method="monte_carlo",
        seed=None,
        **kwargs,
    ):
        """Generate an equilibrium spectrum with uncertainty propagation.

        This method leverages the :mod:`radis.tools.uncertainty` module
        to propagate uncertainties in the experimental conditions and
        line parameters, returning a spectrum with confidence bands.
        ...
        Tgas : float
            Gas temperature (K).
        n_samples : int
            Number of Monte Carlo samples to draw. Default 200.
        uncertainty_source : str
            Source of line parameter uncertainties (e.g. ``'hitran'``).
        confidence_level : float
            Confidence level for the calculated bands (default 0.95).
        method : str
            Sampling method: ``'monte_carlo'`` or ``'latin_hypercube'``.
        seed : int or None
            Random seed for reproducibility.
        **kwargs
            Other parameters passed to :meth:`~radis.lbl.factory.SpectrumFactory.eq_spectrum`
            (e.g., ``mole_fraction``, ``path_length``, etc.) and
            considered as deterministic conditions (unless registered
            in the uncertainty model).

        Returns
        -------
        Spectrum
            An equilibrium spectrum with the ``_confidence`` attribute
            populated, containing the mean spectrum and confidence bands.
        ...
        *(Phase 1 Stub)* Full line-parameter Monte Carlo propagation
        ...
        --------
        :meth:`radis.lbl.factory.SpectrumFactory.eq_spectrum`,
        :class:`radis.tools.uncertainty.UncertaintyPropagator`
        """
        from radis.tools.uncertainty import UncertaintyPropagator

        uq = UncertaintyPropagator(self)
        if uncertainty_source:
            uq.add_line_parameter_uncertainty(source=uncertainty_source)

        # Full integration to be completed in Phase 2.
        # For now, just generate the nominal spectrum and attach
        # an empty/stub confidence band, or use the propagator
        # if only condition uncertainties were provided.
        ...
        # Build nominal spectrum (we'll just use the mean spectrum
        # generated by the propagator as the nominal for now)
        s = self.eq_spectrum(Tgas=Tgas, **kwargs)

        # Attach confidence bands for all computed quantities
        # Currently, the Propagator only runs for 'radiance_noslit' by default,
        # but in Phase 2 it will handle all quantities.
