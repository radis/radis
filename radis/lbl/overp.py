...
from radis.spectrum.rescale import (rescale_abscoeff, rescale_absorbance, rescale_emisscoeff,
                                    rescale_radiance_noslit, rescale_transmittance_noslit)
...
class LevelsList(object):
    ...
    __slots__ = ['Tvib_ref', 'Trot_ref', 'vib_distribution_ref',
                 'mole_fraction_ref', 'path_length_ref',
                 'sorted_bands', 'cum_weight', 'E_bands',
                 'bands_ref', 'verbose', 'bands', 'parfunc', 'levelsfmt',
                 'lvl_index', 'vib_levels', 'copy_lines', 'Qref', 'Qvib_ref']

    def __init__(self, parfunc, bands, levelsfmt, sortby='Ei', copy_lines=False, verbose=True):
        ''' 
        Parameters
        ----------
        ...
        for br, s in bands.items():
            if not is_spectrum(s):
                raise ValueError('`bands` must be a list of Spectrum objects. ' +
                                 'Got {0}'.format(type(s)))
        ...
        Qvib = self._calc_vib_populations(Tvib=self.Tvib_ref,
                                          vib_distribution=self.vib_distribution_ref)

        self.Qref = parfunc.at_noneq(Tvib_ref, Trot_ref,
                                     vib_distribution=vib_distribution_ref)  # TODO: add overpopulations in partition function
        self.Qvib_ref = Qvib
...
#            del self.bands0[br].lines
        ...
        # Initialize bands
        self.bands_ref = {br: band.copy(
            copy_lines=self.copy_lines) for br, band in bands.items()}
...
#            row['viblvl'] = '({p},{c})'.format(**dict([(k,int(row[k])) for k in ['p', 'c']]))
        ...
        for k in lvl_index:  # Note: do not fill directly in frmokeys or you'll get the same shared list
            lvl_index[k] = {'bands_where_low': [],
                            'bands_where_up': []}
    ...
    def _calc_vib_populations(self, Tvib, vib_distribution='boltzmann',
                              overpopulation=None):
        ...
        if is_float(Tvib):
            ...
            if vib_distribution == 'boltzmann':
                nvibQvib = g * exp(-hc_k*E_vib1/Tvib1) * \
                    exp(-hc_k*E_vib2/Tvib2) * exp(-hc_k*E_vib3/Tvib3)
            else:
                raise NotImplementedError(
                    'vib_distribution: {0}'.format(vib_distribution))
        ...
        if overpopulation != {}:
            for k, ov in overpopulation.items():
                nvibQvib.loc[k] *= ov
                #TODO: test
            warn('NotImplemented: partition function overpopulation correction not tested')
...
#        self.bands_ref = new_bands
    ...
    def eq_spectrum(self, Tgas, overpopulation=None, mole_fraction=None, path_length=None,
        ...
        '''
        s = self.non_eq_spectrum(Tgas, Tgas, overpopulation=overpopulation,
                                 mole_fraction=mole_fraction, path_length=path_length,
    ...
    def non_eq_spectrum(self, Tvib=None, Trot=None, Ttrans=None,
        ...
        bands = {br: bands_ref[br].copy(
            copy_lines=self.copy_lines) for br in bands_ref}
        ...
        if Trot != self.Trot_ref:
            raise ValueError('Trot {0} doesnt match the reference Trot: {1}'.format(Trot,
                                                                                    self.Trot_ref))
...
#                pop_correction[br] = exp(-E_bands[br]*hc_k/Tvib)/exp(-E_bands[br]*hc_k/Tvib_ref)
        ...
        Qvib = self._calc_vib_populations(Tvib=Tvib, vib_distribution=vib_distribution,
        ...
        if overpopulation is not None:
            Q, _, _ = self.parfunc.at_noneq(Tvib, Trot, overpopulation=overpopulation,
                                            returnQvibQrot=True, update_populations=False)
        else:
        ...
        if mole_fraction != mole_fraction_ref:
            s.rescale_mole_fraction(mole_fraction, mole_fraction_ref,
        ...
        if save_rescaled_bands:
            for br, band in bands.items():          # type(band): Spectrum
                if mole_fraction != mole_fraction_ref:
                    band.rescale_mole_fraction(mole_fraction, mole_fraction_ref,
                                               ignore_warnings=True,  # mole_fraction is 'N/A'
                                               force=True)
                if path_length != path_length_ref:
                    band.rescale_path_length(
                        path_length, path_length_ref, force=True)
            self.bands = bands
...
def rescale_updown_levels(spec, new_nu, old_nu, new_nl, old_nl,
...
#                                ' `from_mole_fraction`)'),
    ...
    if 'abscoeff' in recompute:
        rescaled, units = rescale_abscoeff(spec, rescaled, initial,     # TODO: remove rescaled = ... Dict is mutable
                                           old_nl, new_nl, path_length,
                                           waveunit, units, extra, true_path_length)

    if 'emisscoeff' in recompute:
        rescaled, units = rescale_emisscoeff(spec, rescaled, initial,
                                             old_nu, new_nu, path_length,
...
if __name__ == '__main__':
    ...
    from neq.test.spec.test_overp import run_testcases
