...
def test_vibrational_levels_labelling(verbose=True, *args, **kwargs):
    ...
    if verbose:
        ...
        printm('... CO, HITRAN format (v):\t\t',
               vib_lvl_name_hitran_class1(10))

        printm('... CO2, HITRAN format (v1,v2,l2,v3):\t\t',
               vib_lvl_name_hitran_class5(2, 1, 1, 3))
        printm('... CO2, HITRAN format, short (v1v2l2v3):\t',
               vib_lvl_name_hitran_class5_short(2, 1, 1, 3))

        printm('... CO2, CDSD format (p,c):\t\t', vib_lvl_name_cdsd_pc(14, 1))
        printm('... CO2, CDSD format (p,c,N):\t\t',
               vib_lvl_name_cdsd_pcN(14, 1, 1))
