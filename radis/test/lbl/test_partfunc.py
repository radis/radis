...
def test_calculatedQ_match_HAPI(vmax=11, jmax=300, plot=False, verbose=True,
    ...
    db = PartFunc_Dunham(S, vmax=vmax, vmax_morse=vmax_morse,
                         Jmax=jmax, use_cached=False)  # , ZPE=1081.584383)
    assert not db.use_cached

#    if plot: db.plot_states()

    hapi = PartFuncHAPI(M=5,  # CO
                        I=1,  # isotope
                        )
...
def test_CDSD_calc_vs_ref(warnings=True, verbose=True, *args, **kwargs):
    ...
    try:
        ...
        assert np.isclose(Qf.at_noneq(3000, 3000),
                          114689.88454184022, rtol=0.001)
        assert np.isclose(Qf.at_noneq(3000, 3000, overpopulation={'(0,1)': 3},
                                      returnQvibQrot=True)[0],
                          120053.34252537244, rtol=0.001)
...
def test_recompute_Q_from_QvibQrot_Dunham_Evib3_Evib12Erot(verbose=True, warnings=True, *args, **kwargs):
...
#@pytest.mark.fast   # (very fast only once the cached database has been generated, else decently fast)
def test_recompute_Q_from_QvibQrot_Dunham_Evib123_Erot(verbose=True, warnings=True, *args, **kwargs):
    '''     Calculate vibrational and rotational partition functions:
...
def test_Q_1Tvib_vs_Q_3Tvib(T=1500, verbose=True, warnings=True, *args, **kwargs):
    ...
    if verbose:
        printm(
            'Tested Q in 1-Tvib vs Q in 3-Tvib modes (T={0:.0f}K): OK'.format(T))
