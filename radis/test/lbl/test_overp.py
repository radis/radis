...
def test_direct_overpopulation_vs_recombined_bands(verbose=True, plot=False, warnings=True,
    ...
    try:
        ...
        if plot:
            plot_diff(sref, s_recombined, var='radiance_noslit', label1='ab initio', label2='recombined bands',
                      title='x_CO2 = 1e-3')

        assert np.allclose(s_recombined.get_radiance_noslit(), sref.get_radiance_noslit(),
                           rtol=rtol)
        ...
        if verbose:
            printm('Testing x_CO2 = 1: ab initio ~ recombined bands (<{0:.1f}%):\t{1}'.format(
                rtol*100, np.allclose(s_recombined.get_radiance_noslit(), sref.get_radiance_noslit(),
                                      rtol=rtol)))
...
def test_3Tvib_vs_1Tvib(verbose=True, plot=False, warnings=True, *args, **kwargs):
    ...
    try:
        ...
        for I in iso:
            ...
            assert (energies.Evib == energies.Evib1 +
                    energies.Evib2+energies.Evib3).all()
        if verbose:
        ...
        if plot:
            s1.plot('transmittance_noslit', wunit='cm-1', color='k', lw=3,
                    label='Tvib = {0}K'.format(T))

            s3.plot('transmittance_noslit', wunit='cm-1', color='r', lw=3,
                    nfig='same', label='Tvib1 = Tvib2 = Tvib3 = {0}K'.format(T))

        assert s1.compare_with(s3, spectra_only=True,
