...
def test_parallel(plot=False, verbose=True, warnings=True, *args, **kwargs):
    ...
    try:
        ...
        # .. parallel
        pf = ParallelFactory(wavelength_min=4165, wavelength_max=4200,
                             cutoff=cutoff,
                             db_use_cached=True,
                             lvl_use_cached=True,
                             isotope='1',
        ...
        # ... non parallel
        sf = SpectrumFactory(wavelength_min=4165, wavelength_max=4200,
                             cutoff=cutoff,
                             db_use_cached=True,
                             lvl_use_cached=True,
                             isotope='1,2',
        ...
        s_sequential = []
        for Troti in Trot:
            s_sequential.append(sf.non_eq_spectrum(Tvib, Troti, path_length=path_length,
                                                   mole_fraction=mole_fraction))
        t1 = time() - t1
        ...
        for i in range(len(Trot)):
            assert s_parallel[i].compare_with(s_sequential[i], spectra_only=True,
                             plot=False)
        if verbose:
...
#        assert t0 > t1
        if t0 > t1:
            warn('Parallel version ({0:.1f}s) is slower than the Serial version ({1:.1f}s) !'.format(
                    t0, t1))
...
def DISCARDED_test_parallel_internal(plot=False, verbose=True, warnings=True, *args, **kwargs):
    ...
    try:
        ...
        # .. parallel
        sfp = SpectrumFactory(wavelength_min=4377.134, wavelength_max=4377.9,
                              pressure=0.020,   # bar
                             cutoff=cutoff,
                             db_use_cached=True,
                             lvl_use_cached=True,
        ...
        s_parallel = sfp.non_eq_spectrum(Tvib, Trot, path_length=path_length, 
                                        mole_fraction=mole_fraction)
        s_parallel.name = 'Parallel ({0:.1f}s)'.format(s_parallel.conditions['calculation_time'])

        # ... non parallel
        sf = SpectrumFactory(wavelength_min=4377.134, wavelength_max=4377.9,
                              pressure=0.020,   # bar
                             cutoff=cutoff,
                             db_use_cached=True,
                             lvl_use_cached=True,
        ...
        s_sequential = sf.non_eq_spectrum(Tvib, Trot, path_length=path_length,
                                                   mole_fraction=mole_fraction)
        s_sequential.name = 'Sequential ({0:.1f}s)'.format(s_sequential.conditions['calculation_time'])
