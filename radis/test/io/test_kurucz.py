...
def test_partfn():
    kurucz = AdBKurucz()
    with patch.object(kurucz, 'pfdat') as mock_pfdat, \
        patch.object(kurucz, 'pfTdat') as mock_pfTdat:
        # Assuming partition function values for corresponding temperatures
        mock_pfdat.loc.__getitem__.return_value = pd.Series([2.0, 2.5, 3.0, 3.5, 4.0, 4.5])
        # Temperature range from 2000K to 3000K
        mock_pfTdat.values.flatten.return_value = np.array([2000.0, 2200.0, 2400.0, 2600.0, 2800.0, 3000.0])
        key = 'H_I'
        T = 2500.0
