...
def test_read_kurucz():
    ...
    a = df.loc[5, ['orig_wavelen', 'loggf', 'species', 'El', 'jl', 'labellower', 'Eu', 'ju', 'labelupper', 'gamRad', 'gamSta', 'gamvdW', 'iso']]#.to_numpy()
    b = np.array([504403.3610,-7.259,'40.00', 35456.250, 2.0, 'd3p w3P', 35476.070,3.0, '5s4F6s e5F', 8.41,-5.27,-7.53, 0], dtype='object')
    assert (a == b).all()
    a = df.loc[7, ['orig_wavelen', 'loggf', 'species', 'El', 'jl', 'labellower', 'Eu', 'ju', 'labelupper', 'gamRad', 'gamSta', 'gamvdW', 'iso']]#.to_numpy()
