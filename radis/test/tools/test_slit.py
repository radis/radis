...
def test_all_slit_shapes(FWHM=0.4, verbose=True, plot=True, close_plots=True, *args, **kwargs):
    ...
    # ... trapezoidal
    s.apply_slit((FWHM*0.9, FWHM*1.1), unit='nm',
                 shape='trapezoidal', plot_slit=plot)
    assert np.isclose(get_FWHM(*s.get_slit()), FWHM, atol=2*wstep)
...
def test_against_specair_convolution(plot=True, close_plots=True, verbose=True, debug=False, *args, **kwargs):
    ...
    if plot:
        fig = plt.figure(
            fig_prefix+'SPECAIR convoluted vs SPECAIR non convoluted')
    ...
    ws, Is = np.loadtxt(getTestFile(
        'calc_N2C_spectrum_Trot1200_Tvib3000_slit0.1.txt')).T  # Specair units: mW/cm2/sr
    if plot:
...
def test_normalisation_mode(plot=True, close_plots=True, verbose=True, *args, **kwargs):
    ...
    if verbose:
        print(
            ("radiance unit ({0}) is homogeneous to 'mW/cm2/sr': OK".format(s.units['radiance'])))
...
def test_slit_energy_conservation(verbose=True, plot=True, close_plots=True, *args, **kwargs):
    ...
    if plot:
        ...
        s.plot('radiance_noslit', nfig=fig.number,
