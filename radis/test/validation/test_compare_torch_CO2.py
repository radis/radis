...
def test_compare_torch_CO2(verbose=True, plot=False, save=False, warnings=True,
    ...
    try:
        ...
        conds = pd.read_csv(getValidationCase('test_compare_torch_CO2_data/test_compare_torch_CO2_conditions_JTHT2003.dat'),
...
#                             add_info=['Tgas'])
    ...
    #    sf.init_databank('CDSD-4000')
        ...
        # CO
        sfco = ParallelFactory(wmin, wmax,
                               mole_fraction=None, path_length=None,
                               molecule='CO',
                               isotope='1,2',
                               parallel=False,
...
#                'test_compare_torch_CO_SpectrumDatabase_HITEMP_1e25', add_info=['Tgas'])
        ...
        # Calculate all slabs
        # .. CO2 at equilibrium
        slabsco2 = sf.eq_spectrum(list(conds.T_K), mole_fraction=list(conds.CO2), path_length=slab_width)
        ...
        stot_corr = experimental_spectrum(wtot, Itot-I0spectro,  # hack to remove
        ...
        # ------
        exp = pd.read_csv(getValidationCase('test_compare_torch_CO2_data/test_compare_torch_CO2_spectrum_JTHT2003.dat'),
                          skiprows=5, delim_whitespace=True)
        exp.w /= 10              # Angstrom to nm
        exp = exp[(exp.w > wlmin) & (exp.w < wlmax)]
        sexp = experimental_spectrum(exp.w, exp.I, Iunit='mW/cm2/sr',
                                     conditions={'medium': 'air'})

        if plot:

            sexp.plot('radiance', wunit='nm', Iunit=unit,
                      lw=2, label='Packan 2003')

            # Plot calculated
            # ----------

            stot.plot('radiance', wunit='nm', Iunit=unit, nfig='same', color='r',
                      ls='--', label='All: CO$_2$+CO')

            # Plot internal spectrometer emission
            s0spectro.plot('radiance', wunit='nm', Iunit=unit, nfig='same', color='b',
                           label='CO$_2$ in Spectrometer')

            stot_corr.plot('radiance', wunit='nm', Iunit=unit, nfig='same', color='r',
                           lw=2, label='All-CO$_2$ in Spectro', zorder=10)

            sco.plot('radiance', wunit='nm', Iunit=unit, nfig='same', color='g',
                     ls='-', label='CO')
...
if __name__ == '__main__':
    printm('test_compare_torch_CO2:',
           test_compare_torch_CO2(plot=True, use_cache=True))
