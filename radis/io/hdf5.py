    ...
    def load(self, fname, columns, where=None, **store_kwargs) -> pd.DataFrame:
        ...
        if self.engine == "pytables":
            ...
            try:
                ...
                raise OSError(
                    f"Cannot read {fname} with Vaex's HDF5 library. It may be a file generated by pytables (row-based). Try using engine='pytables' in `hdf2df` or `fetch_hitemp` ;  delete the file to re-download and re-parse it (this may take a lot of time !) ;  or set `import radis; radis.AUTO_UPDATE_DATABASE = True` in your script to auto-update to Vaex HDF5 file"
                ) from err
