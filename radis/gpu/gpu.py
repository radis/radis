...
class gpuApp(GPUApplication):
    def __init__(   self,  
       ...
       Returns
        ...
        self.init_d = GPUBuffer(sizeof(initData_t), usage='uniform', binding=0)  #host_visible, device_local, (host_cached)
        self.iter_d = GPUBuffer(sizeof(iterData_t), usage='uniform', binding=1)  #host_visible, device_local, (host_cached) 
        ...
        for arr in database_arrays:
            byte_offset += self.database_d.copyToBuffer(arr, device_offset=byte_offset, chunksize=32*1024*1024)
            
        self.S_klm_d = GPUBuffer(fftSize=self.init_h.N_v_FT, binding=3) #req: large, device_local
        ...
        self.appendCommands([
            ...
            self.cmdScheduleShader('cmdFillLDM.spv', (self.init_h.N_lines // N_tpb + 1, 1, 1), threads),
            self.cmdAddTimestamp('FFT fwd'),
            self.cmdFFT(self.S_klm_d, name='FFTa'),
            self.cmdAddTimestamp('selfly conv.'),
            self.cmdScheduleShader('cmdapplyLineshapes.spv', (self.init_h.N_x_FT // N_tpb + 1, 1, 1), threads),
