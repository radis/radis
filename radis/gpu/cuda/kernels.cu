...
__global__ void fillLDM(
    ...
    LOOP(blockIdx.x, gridDim.x){
        LOOP(threadIdx.x, blockDim.x){
            ...
            unsigned int iso = *reinterpret_cast<unsigned int*>(&database_d[i]);
            ...
            //Calc wG
            float log_wGi = logf(v0) + init_d.log_c2Mm[iso] + iter_d.hlog_T;
            ...
            float log_wLi = logf(gamma) + iter_d.log_2p + na * iter_d.log_rT;
            ...
            float Si = iter_d.N * iter_d.x[0] * S0 * (expf(-iter_d.c2T * El) - expf(-iter_d.c2T * (El + v0))) / iter_d.Q[iso];
            ...
            ADD(&S_klm_d[k0i * N_G * N_L + l0i * N_L + m0i], Sv0i * aV00i);
            ADD(&S_klm_d[k0i * N_G * N_L + l0i * N_L + m1i], Sv0i * aV01i);
            ADD(&S_klm_d[k0i * N_G * N_L + l1i * N_L + m0i], Sv0i * aV10i);
            ADD(&S_klm_d[k0i * N_G * N_L + l1i * N_L + m1i], Sv0i * aV11i);
            ADD(&S_klm_d[k1i * N_G * N_L + l0i * N_L + m0i], Sv1i * aV00i);
            ADD(&S_klm_d[k1i * N_G * N_L + l0i * N_L + m1i], Sv1i * aV01i);
            ADD(&S_klm_d[k1i * N_G * N_L + l1i * N_L + m0i], Sv1i * aV10i);
            ADD(&S_klm_d[k1i * N_G * N_L + l1i * N_L + m1i], Sv1i * aV11i);
